...
Процедура ЗаполнитьНачалоИОкончаниеДня(Объект)
...
Процедура РаботыСодержаниеРаботНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(мОтбор) Тогда
		Отбор.Вставить("Наименование", мОтбор);
	КонецЕсли; 
	
	Параметр.Вставить("Отбор", Отбор);

	ОткрытьФорму("Справочник.ТБС_ОписаниеРабот.ФормаВыбора",Параметр,Элементы.Работы);
