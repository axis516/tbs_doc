...
Функция ПолучитьСписокДоступныхФункций() Экспорт
	...
	////"---------
	ДоступныеФункции.Добавить(
		"ШаблоныДокументов.ВсеРуководителиАдресата_Адресат(Объект)",
		НСтр("ru = 'Все руководители адресата'"));
	////----------"
...
////"---------
//	Получает адресата и руководителей адресата.
//
//	Параметры:
//		ДокументОбъект - СправочникОбъект.ВнутреннийДокумент,
//              СправочникОбъект.ВходящийДокумент,
...
//		Массив
//
Функция ВсеРуководителиАдресата_Адресат(ДокументОбъект) Экспорт
	...
	Если ТипЗнч(ДокументОбъект.Ссылка) = Тип("СправочникСсылка.ВходящиеДокументы") Или 
		 ...
		 АвторДокумента = ДокументОбъект.Адресат;
		 
		 Если ЗначениеЗаполнено(АвторДокумента) Тогда
			 МассивПользователей.Добавить(АвторДокумента);
		 КонецЕсли; 
