	...
	Если ИменаПредметовДляФункций <> Неопределено Тогда
		Если ИменаПредметовДляФункций.Количество() > 0 Тогда
			Для Каждого ИмяПредмета Из ИменаПредметовДляФункций Цикл
				...
				////"---------
				ДоступныеФункции.Добавить("ШаблоныБизнесПроцессов.ВсеРуководителиАдресата(Объект, ИмяПредмета)",
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1.Все руководители адресата'"),Строка(ИмяПредмета)));
				////----------"
		...
		ИначеЕсли ВключатьНедоступные Тогда
			...
			ДоступныеФункции.Добавить("ШаблоныБизнесПроцессов.СекретарьМероприятия(Объект, ИмяПредмета)",
				...
				////"---------
			ДоступныеФункции.Добавить("ШаблоныБизнесПроцессов.ВсеРуководителиАдресата(Объект, ИмяПредмета)",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1.Все руководители адресата'"),СтрокаИмениПредмета), Истина);
				////----------"	
		КонецЕсли;
	КонецЕсли;
...
// Возвращает всех руководителей адресата
//
// Параметры:
//   БизнесПроцессОбъект - БизнесПроцессОбъект.<Тип бизнес процесса> - процесс, для которого
//                         вычисляется автоподстановка
...
//     - СправочникСсылка.Пользователи
//
Функция ВсеРуководителиАдресата(БизнесПроцессОбъект, ИмяПредмета) Экспорт 
	
	МассивПользователей = Новый Массив;
	
	РуководительАвтораДокумента = НепосредственныйРуководительАвтораДокумента(БизнесПроцессОбъект, ИмяПредмета);
	
	Если ЗначениеЗаполнено(РуководительАвтораДокумента) Тогда
		МассивПользователей.Добавить(РуководительАвтораДокумента);
	КонецЕсли; 
	...
	Если СтрокаПредмета <> Неопределено Тогда
		...
		Адресат = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаПредмета.Предмет, "Адресат");
		...
		|	РегистрСведений.ПодчиненностьСотрудников КАК ПодчиненностьСотрудников
		|ГДЕ
		|	ПодчиненностьСотрудников.Подчиненный = &Подчиненный";
		Запрос.УстановитьПараметр("Подчиненный", Адресат);
		Выборка = Запрос.Выполнить().Выбрать();
		
		УстановитьПривилегированныйРежим(Истина);
		
		Пока Выборка.Следующий() Цикл
			МассивПользователей.Добавить(Выборка.Руководитель);
		КонецЦикла;
	КонецЕсли;	
